# AGENTS.md — Cooja-NG C Emulator

This is the single source of agent guidance for the repo; `CLAUDE.md` imports
it. Edit this file.

## Conventions

Two rules govern nearly every change here (see
...
- **Ports, not type switches.** The kernel is ports-and-adapters: never branch
  on a node's concrete type. Add an op to the relevant vtable (`sim_mote_ops_t`,
  `sim_service_ops_t`, `sim_medium_ops_t`) instead. Phase 10 deliberately
  removed the last `NODE_MSP430`/`NODE_ARM` switches from the runner — don't
  reintroduce them.
...
| Question | Start here |
|---|---|
| How does the radio actually move bytes? | `src/sim/sim_radio_bus.c` + [`docs/radio-medium.md`](docs/radio-medium.md) |
| What does the JSON config support? | [`docs/test-format.md`](docs/test-format.md), `src/sim/sim_config.c`, `configs/` |
| Adding a new chip, peripheral, or board? | [`docs/porting-a-device.md`](docs/porting-a-device.md) |
| How is a Cooja `.csc` test parsed? | `tools/csc2json.py` → `src/sim/sim_config.c` → `src/common/js_test_engine.c` |
| Writing a plugin (service, medium, UI panel)? | `include/sim/csim_plugin.h`, [`docs/design/ui-plugins.md`](docs/design/ui-plugins.md), `plugins/` |
| Subsystem overview with diagrams | [`docs/architecture.md`](docs/architecture.md) |
| Zephyr / RIOT firmware support | [`docs/zephyr.md`](docs/zephyr.md), [`docs/riot.md`](docs/riot.md) |
| Per-device port status | `devices/<platform>/` |
...
./build/test_runner multinode         # 1 node, nullnet-broadcast, 20s (see "Stale shortcuts")
...
# Chip-driver, radio, and serializer unit suites
...
./build/test_runner radio-bus               # 120 RF-delivery tests (Phase 5 guardrail)
./build/test_runner timeline                # 76 event-serializer tests
./build/test_runner all                     # every suite that opts into "all"
...
Multinode options: `-t ms` (sim duration), `-n nodes` (node count), `-d ms`
(per-node startup-delay spread), `-q` (quiet), `-v` (verbose).
...
`test_runner`'s mode selector does not drill below the suite level, and there is
no name/substring filter — don't go looking for one. To narrow further, edit the
static case array in the relevant `test/test_*.c`, or use `-v` to get per-test
names and read past the failure. For firmware behaviour, a JSON config under
`configs/` is usually the faster loop than a C test.

### Stale shortcuts

`./build/test_runner multinode` with no firmware defaults to **1** node of
`firmware/sky/nullnet-broadcast.sky` for 20 s — it does not hang (an old note in
README's "Known issues" says otherwise), but it is also not a 2-node test. The
`zoul-firefly-*`, `nrf52840-*` and `nrf54l15-*` wrappers force `-n 2` when you
omit it; plain `multinode` and `arm-multinode` do not. Pass firmware explicitly
or use a JSON config when the node count matters.

### What CI enforces

`.github/workflows/test.yml` runs on Ubuntu + macOS. Beyond the unit, firmware
and multinode suites above, it gates three properties that are easy to break
without noticing locally — run them before pushing:

```sh
tools/check-determinism.sh multinode firmware/sky/udp-server.sky firmware/sky/udp-client.sky -t 20000
tools/check-config-equivalence.sh configs/chain-4node-sky.json configs/chain-4node-sky-v2.json
make plugins && tools/check-plugin.sh
```

Determinism (same run twice → byte-identical) is the one most often broken by an
innocuous-looking scheduling or iteration-order change. CI also asserts on
firmware-reported reception counts for the nRF52840 and Zephyr runs rather than
on the runner's exit status — the runner exits 0 regardless.

End-to-end against upstream Contiki-NG: `tools/run-cooja-tests.sh` (93 tests,
~10–15 min warm; needs `CONTIKI_DIR`).

### Tuning and environment variables

| Variable | Default | Effect |
|---|---|---|
| `MSPSIM_JIT_THRESHOLD` | `100` | Block executions before JIT compilation |
| `MSPSIM_JIT_INBLOCK_CHECKS` | `1` | IRQ/event checks inside JIT blocks; `0` for max throughput |
| `CSIM_SERIAL_TX_LATENCY_MS` | `40` | Host-link latency on mote→host serial; `0` disables. Required by the native border router — see `src/sim/sim_serial_bridge.c` |
| `CSIM_TRACE_TSCH_ACK` | unset | Verbose CC2420 ACK timing trace for TSCH debugging |
| `CSIM_TRACE_EVENT_SPIN` | unset | Trace same-time event spin-loop iterations |
| `CSIM_GPIO_TRACE` | unset | Timestamped GPIO OUT changes (nRF54L15) |
| `NRF54L_RADIO_TRACE` | unset | nRF54L15 RADIO state/event/INTENSET trace; filter with `NRF54L_RADIO_NODE=<cpu-tag>` |
| `CSIM_GE_AVG_DROP`, `CSIM_GE_BURST_LEN` | — | Gilbert-Elliott medium knobs |
| `CONTIKI_DIR` | `../contiki-ng` | Where `make cooja-tests` and the firmware builders look |
...
- `MSPSIM_JIT_INBLOCK_CHECKS` env var (default **1**, i.e. on — needed for timer
  events in tight loops; set `0` for max throughput): interrupt checks inside
  JIT blocks
