/*
 * CC2538 SoC peripheral bundle — implementation.
 *
 * Owns the lifecycle (init / destroy / console hookup) of every
 * CC2538-specific peripheral instance, plus the small set of stubs the
 * SoC's address map references but for which there is no full model yet
 * (Flash control, ANA regs, SOC-ADC, I2C, USB, uDMA).
 *
 * Plugged into the SoC-agnostic platform via `cc2538_soc_ops`.
 */
 ...
 * goes wrong. */
static uint8_t cc2538_cc1200_ssi_exchange(void *user_data, uint8_t mosi) {
    arm_platform_t *plat = (arm_platform_t *)user_data;
    cc2538_soc_t *soc = arm_platform_cc2538(plat);
    return cc1200_spi_exchange(&soc->cc1200, mosi);
}
...
static void cc2538_cc1200_gpio_output(void *user_data, int port,
    ...
    if (cfg->cc1200_csn.port == port) {
        ...
        if (old_low != new_low) {
            cc1200_set_csn(&soc->cc1200, new_low);
        }
    }
    ...
    if (cfg->cc1200_reset.port == port) {
        ...
        bool new_low = (new_val & mask) == 0;
        if (old_low != new_low) {
            cc1200_set_reset(&soc->cc1200, new_low);
        }
    }
}

/* ============================================================
 * Peripheral stubs — small register windows the firmware reads
 * from / writes to but for which there is no full peripheral
 * model yet.
 * ============================================================ */
...
static void cc2538_soc_init(arm_platform_t *plat) {
    ...
    /* UARTs */
    cc2538_uart_init(&soc->uart0, &plat->cpu, 0x4000C000, 5);
    cc2538_uart_init(&soc->uart1, &plat->cpu, 0x4000D000, 6);
    cc2538_uart_set_nvic(&soc->uart0, &plat->nvic);
    cc2538_uart_set_nvic(&soc->uart1, &plat->nvic);

    /* GPIO */
    cc2538_gpio_init(&soc->gpio, &plat->cpu);

    /* Build the CPU-agnostic host vtable used by off-SoC chip drivers. */
    plat->host.cpu            = &plat->cpu;
    plat->host.gpio           = &soc->gpio;
    ...
    /* IOC */
    cc2538_ioc_init(&soc->ioc, &plat->cpu);

    /* GPTimers */
    cc2538_gptimer_init(&soc->gptimer[0], &plat->cpu, GPTIMER0_BASE, 19, 20, 0);
    cc2538_gptimer_init(&soc->gptimer[1], &plat->cpu, GPTIMER1_BASE, 21, 22, 1);
    cc2538_gptimer_init(&soc->gptimer[2], &plat->cpu, GPTIMER2_BASE, 23, 24, 2);
    cc2538_gptimer_init(&soc->gptimer[3], &plat->cpu, GPTIMER3_BASE, 35, 36, 3);

    /* RF Core */
    cc2538_rfcore_init(&soc->rfcore, &plat->cpu, &plat->nvic);
    ...
    cc2538_sleeptimer_init(&soc->sleeptimer, &plat->cpu, &plat->nvic, 145);
    ...
    cc2538_ssi_init(&soc->ssi0, &plat->cpu, SSI0_BASE, CC2538_SSI_BUS_0);
    cc2538_ssi_init(&soc->ssi1, &plat->cpu, SSI1_BASE, CC2538_SSI_BUS_1);
    ...
    if (config->has_cc1200) {
        cc1200_init(&soc->cc1200, &plat->host);
        cc1200_set_gdo0_pin(&soc->cc1200,
                             config->cc1200_gdo0.port,
                             config->cc1200_gdo0.pin);
        if (config->cc1200_gdo2.port >= 0) {
            cc1200_set_gdo2_pin(&soc->cc1200,
                                 config->cc1200_gdo2.port,
                                 config->cc1200_gdo2.pin);
        }
        /* Route the chosen SSI bus to the CC1200 driver. */
        cc2538_ssi_t *bus = (config->cc1200_ssi == 1) ? &soc->ssi1 : &soc->ssi0;
        cc2538_ssi_set_exchange_callback(bus, cc2538_cc1200_ssi_exchange, plat);
        /* Tap GPIO output transitions for CSn + RESET. */
        cc2538_gpio_set_output_callback(&soc->gpio, cc2538_cc1200_gpio_output, plat);
    }
}

static void cc2538_soc_destroy(arm_platform_t *plat) {
    cc2538_soc_t *soc = (cc2538_soc_t *)plat->soc;
    if (!soc) return;
    free(soc->udma);
    free(soc->usb);
    free(soc);
...
static void cc2538_soc_set_console(arm_platform_t *plat,
                                    arm_uart_tx_callback cb, void *user_data) {
    cc2538_soc_t *soc = (cc2538_soc_t *)plat->soc;
    if (plat->config->console_uart == 0) {
        cc2538_uart_set_callback(&soc->uart0, cb, user_data);
    } else {
        cc2538_uart_set_callback(&soc->uart1, cb, user_data);
    }
}

const arm_soc_ops_t cc2538_soc_ops = {
    .name        = "cc2538",
    .init        = cc2538_soc_init,
    .destroy     = cc2538_soc_destroy,
    .set_console = cc2538_soc_set_console,
};
