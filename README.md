...
2. **Multi-node nRF54L15 802.15.4 reception does not work (open bug, cooja-ng radio model).**  Two nRF54L15 nodes never complete RPL/UDP: the sender transmits and the medium delivers the bytes, but the receiver's radio model drops the frame mid-reception, so no frame reaches the stack and the peer never joins the DAG ("Not reachable yet").  Root-caused to two layered bugs in `src/arm/nrf54l15_soc.c`'s receive path — a deferred-disable safety timeout (5 µs) that fires mid-frame because it is far shorter than a frame's air-time, and reception corruption (byte double-delivery) that fails the FCS once the frame does complete.  Confirmed at `v0.1.0` and pre-CRC-fix, so it has never worked and is **not** a regression; the *identical* Contiki-NG firmware routes fine on the nRF52840 model, so this is an emulator issue, not a firmware one.  The FLPR single-node dual-core demo is unaffected.  Detailed diagnosis and fix plan: [`docs/design/release-0.1.1-hardening.md`](docs/design/release-0.1.1-hardening.md) §T3.  The same class of receive-path issue affects the nRF52840 4-node multi-hop chains.
