...
Scales with sim length as the ~15 ms fixed boot cost amortizes — at 60 s it is **714× real-time**.  The FLPR ticks correctly throughout (59 samples over 60 s; last `tick 80` at 60.0 s), so both cores are genuinely running, just sleeping between events.  For context, this exact demo was **~0.15×** before the idle work (FLPR busy-polling the GRTC + M33 in the NOP-spin) — roughly a **~4,700× wall-clock speedup** at 60 s.  This requires the interrupt-driven `rv32emc` `flpr-host` firmware (GRTC-CC3 etimers + `WFI`) together with csim's RV32E `WFI`-as-idle modelling; the older *polled* firmware busy-spins the GRTC at ~0.15×.  See the FLPR row in [Platforms](#platforms) and `docs/design/riscv-vpr-plan.md`.  (Single-hop 2-node nRF54L15 802.15.4 RPL-UDP works; multi-hop chains don't yet — see [Known issues](#known-issues).)
...
2. **Multi-hop (3+ node) chains don't route past the first hop** — nRF54L15 and nRF52840 alike.  *Single-hop* now works end-to-end: the T3 nRF54L15 receive bug (the radio TX-completion event was scheduled off the lagging `sim_time_ns`, collapsing every TX to one cycle and making the DPPI fan-out emit each frame twice — two SFDs on air, so the receiver mis-latched the second SFD as the frame length and every frame failed CRC) is **fixed** by scheduling `tx_end_event` in cycles (`src/arm/nrf54l15_soc.c`); `configs/test-2node-nrf54l15-dk.json` exchanges request/response reliably.  In a 3-node chain the one-hop link works and the far node joins the DAG and sends, but its traffic isn't relayed across the second hop — a separate 6LoWPAN/RPL multi-hop forwarding issue (it also blocks the nRF52840 4-node chains), still open.  Diagnosis: [`docs/design/t3-nrf54l15-rx-plan.md`](docs/design/t3-nrf54l15-rx-plan.md).
