...
- **Nordic nRF52840** — CLOCK/HFCLK/LFCLK (+ STAT/SRC status regs), RTC0/RTC1, TIMER0–4, GPIO/GPIOTE, PPI, RNG, NVMC, FICR (per-node DEVICEID), UART (legacy + UARTE EasyDMA), and a full 802.15.4 RADIO (PACKETPTR EasyDMA, SHORTS, INTENSET, BCMATCH, hardware-style auto-ACK).  **Boots stock Zephyr OS** as well as Contiki-NG — an unmodified `nrf52840dk` `hello_world` prints over the default UARTE console; see [`docs/zephyr.md`](docs/zephyr.md).
